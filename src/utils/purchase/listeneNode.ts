...
async function catchPigEvents(listenAddress: Address, afterLt: bigint) {
  const tc = getTonClient();
  const tac = getTonApiClient();
  const adminWallet = await getAdminWallet(tc);
  const pig = tc.open(
...
export async function listenPigForever() {
  let lastLt = BigInt(0);
  while (true) {
    lastLt = await catchPigEvents(
      Address.parse(process.env.NEXT_PUBLIC_PIG_ADDRESS!),
      lastLt
    );
  }
}